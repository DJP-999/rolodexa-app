...
export type FitInput = {
  ...
  // Recent email/LinkedIn thread topics WITH the dealmaker — first-hand evidence of what this
...
export async function gradeFitBatch(batch: FitInput[], focus: UserFocus): Promise<FitResult[]> {
  ...
  const system =
    "You score how relevant each professional contact is to ONE SPECIFIC dealmaker, 0.0 to 1.0, based STRICTLY on that dealmaker's own stated focus and target counterparties below. Different dealmakers have completely different focuses; grade ONLY against the one given here, and NEVER against any assumed thesis, a generic notion of 'private markets', or a strategy the dealmaker did not state.\n" +
    "THE DEALMAKER:\n" +
    `- Role: ${focus.role?.trim() || "a dealmaker building a high-value professional network"}\n` +
    `- Focus: ${focus.currentFocus?.trim() || "(not specified — grade on general professional value: senior decision-makers, capital sources, and the most direct counterparties for their role)"}\n` +
    ...
    (focus.priorityConnections?.trim()
      ...
      : "") +
    "EVIDENCE PRIORITY — base each score on the dossier in THIS order of authority: (1) [Recent threads with you] — what the contact and dealmaker ACTUALLY transact together (the single strongest signal); (2) the [User's CRM notes] — first-hand, authoritative for this contact's specific intent and wants; (3) the contact's [LinkedIn profile] — their real role, seniority, and trajectory; (4) the [Firm research] / [PitchBook firm intel] brief — what the current firm ACTUALLY does and the specific stage/asset class it invests in; (5) [Facets] only as supporting context. When a [LinkedIn profile] or [Firm research] fact contradicts the stored title or a generic label, TRUST the profile/research; when a live on-thesis [Recent threads with you] or [User's CRM notes] signal contradicts an off-thesis profile, TRUST the threads/notes. When firm research is present, ground the firm's strategy/stage/asset-class in it rather than guessing from the name.\n" +
    "METHOD: Reason about the contact's FIRM (its actual strategy — what it does and what it invests in, its stage/asset class, and its standing) and the person's seniority, then score how DIRECTLY they serve the dealmaker's SPECIFIC stated focus, active mandates, and priority counterparties above. Score as a GRADIENT of closeness to that stated focus — never a binary on/off. Derive the hierarchy FROM the dealmaker's stated focus, whatever it is:\n" +
    "- TOP (0.85-1.0): the most DIRECT counterparties or capital sources for the dealmaker's exact stated focus — firms/people whose core strategy or mandate IS precisely what the dealmaker brokers, raises for, buys, or sells, OR who match one of the dealmaker's stated priority-counterparty types. For a BROKER/intermediary, BOTH sides of their market are TOP: the SELLERS/originators of the exposure they handle AND the BUYERS/capital sources for it. BE DECISIVE: when a contact's firm is a bullseye for the stated focus, or clearly matches a named priority-counterparty type, score 0.93-1.0, NOT 0.8. Do not discount an obvious match into the 0.7s/0.8s for lack of fame or thin data; if the described strategy or counterparty type squarely matches, it is TOP.\n" +
    ...
    "MATCH THE SPECIFIC SUB-STRATEGY, STAGE, AND ASSET CLASS — NOT A KEYWORD. A surface keyword match is NOT a fit: if the focus names a particular asset class, stage, or deal type and the contact operates in a DIFFERENT sub-strategy that merely shares a word, they are NOT top — drop them to MODERATE (or LOW if clearly off). Conversely, a firm that does NOT share the dealmaker's keywords but whose ACTUAL described strategy is a direct counterparty or capital source for the focus IS top. Distinguish stage (early vs growth/late vs buyout), asset class (equity vs credit), and deal type even when they share a label.\n" +
    ...
    "RECENT THREADS WITH THE DEALMAKER ARE DECISIVE EVIDENCE. The '[Recent threads with you]' line lists the actual email/LinkedIn subjects this contact and the dealmaker have exchanged. This is first-hand proof of what they ACTUALLY transact together — it outranks the LinkedIn profile, the firm name, and any generic label. If those threads are about the dealmaker's exact business (e.g. for a secondaries dealmaker: a cap-table transfer, a discounted secondary, an allocation, an SPV, a fund/LP interest, a specific company's shares), this contact is a PROVEN, ACTIVE counterparty in the dealmaker's exact focus — score them TOP (0.9-1.0) EVEN IF their LinkedIn bio looks off-thesis (a different industry, an operator/founder background, or a firm with no public research). A stale or unrelated profile NEVER overrides a live on-thesis deal thread. Only discount when the threads are purely personal/administrative with no business-of-the-dealmaker content.\n" +
    ...
    'Return ONLY JSON {"items":[{"id":"<id>","fit":0.0,"summary":"one line: what they do / invest in","rationale":"where on the gradient and why — name the firm\'s strategy/stage focus and why it sits there relative to THIS dealmaker\'s stated focus and priority counterparties"}]}.';
