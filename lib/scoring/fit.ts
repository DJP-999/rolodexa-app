...
export async function gradeFitBatch(batch: FitInput[], focus: UserFocus): Promise<FitResult[]> {
  ...
  const system =
    ...
    "If the dossier explicitly states the contact invests in or is interested in what the dealmaker focuses on (e.g. 'Deal Interest: Secondaries' for a secondaries-focused dealmaker, or that they seek specific pre-IPO names), treat that as a DIRECT on-thesis match and score HIGH — stated intent from a meeting outweighs a generic 'Venture Capital' industry label.\n" +
